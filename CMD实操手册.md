...
从 Git clone 领取模板、修改代码，到提交并推送自己的远程仓库。本版专门用于 Windows CMD。PowerShell 版继续保留，选择一种终端，把整条流程做完即可。这里沿用原来的 web-work、kit-01-v12 和个人 p1-portfolio，不另建一套项目。
...
首次才复制。已有 p1-portfolio 时保留原工程，直接打开检查；本代码框不会覆盖它。
...
解释：set /p 会停下来等待你的回答，不要把下一框的命令作为展示名输入。
