# 第05课 主题记忆与请求：CMD 全流程实操

从 Git clone 领取模板、修改代码，到提交并推送自己的远程仓库。本版专门用于 Windows CMD。PowerShell 版继续保留，选择一种终端，把整条流程做完即可。这里沿用原来的 web-work、kit-05-v12 和个人 p1-portfolio，不另建一套项目。
...
rem clone 下载资源；branch 指定第 5 课，single-branch 只取这一课，depth 1 只取最新提交。
rem 最后的 kit-05-v12 是下载到电脑后的文件夹名称。
git clone --branch p1-guide-05-v1.2 --single-branch --depth 1 "%courseRepo%" kit-05-v12
```

解释：若提示目录已经存在，先查看其 README 核对课次，确认是本课资源才继续。不要删除已有成果或反复 clone。

成功标志：下载成功并返回提示符，目录内出现 kit-05-v12。
...
if exist "kit-05-v12\starter\package.json" (echo READY: starter found) else (echo STOP: starter not found)
```
...
成功标志：web-work 下出现 p1-before-05.zip。已有备份时先核对是本轮导入前的备份，确认后才继续。
...
成功标志：进入 lesson-05。若分支已存在，先核对本课进度，不重新导入。
...
if defined p1Dirty (
  ...
  if not exist "..\p1-before-05.zip" (
    echo STOP: backup is missing.
  ) else (
    if not exist "..\kit-05-v12\starter\package.json" (
      echo STOP: starter is missing.
    ) else (
      rem E 包含子目录，I 指定目标目录，H 保留隐藏项，Y 替换模板同名文件。
      xcopy "..\kit-05-v12\starter\*" "." /E /I /H /Y
    )
  )
)
```
...
在文件资源管理器中解压 p1-before-05.zip，只把自己的展示名、介绍、项目数据、链接和 README 记录迁移回来，具体位置见第 5 步。不要用旧工程整包覆盖新模板。
...
正常联网显示用 projects.json；请求失败时备用数据用 data.js，两处都要填写自己的作品。JSON 使用双引号，最后一项后不能留逗号。 先只迁移内容，再补本课 TODO。

### 5.2 修改 scripts/theme-service.mjs

打开 p1-portfolio/scripts/theme-service.mjs。搜索 TODO-L5，找到下面这段原文。只选中这段，不要选中相邻函数或其他区块。
...
代码解释：先把输入统一为有效主题，再用 setItem 保存；try/catch 捕获存储失败，让网页仍能使用本次选择的主题。

按 Ctrl+S 保存，回浏览器 Ctrl+R 刷新，再做下方检查。

### 5.3 修改 scripts/theme-service.mjs
...
代码解释：三元表达式表示：当前为 dark 时返回 light，否则返回 dark。

按 Ctrl+S 保存，回浏览器 Ctrl+R 刷新，再做下方检查。

### 5.6 逐项观察结果

点击主题按钮变为深色，Ctrl+R 刷新后仍为深色，再点一次变浅色。按 F12 打开开发者工具，在 Network 中搜索 projects.json，刷新后应看到请求；地址末尾加 ?empty=1 检查空态，加 ?fail=1 检查请求失败和内置数据提示；最后删除查询参数恢复正常网址。本课请求逻辑已在 app.js 和 scripts/project-loader.mjs 提供，先读它们并验证，不要求重写。两段检查分别回 PPT 第 25、31 页。
