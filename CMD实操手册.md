# 第04课 筛选与排序：CMD 全流程实操

从 Git clone 领取模板、修改代码，到提交并推送自己的远程仓库。本版专门用于 Windows CMD。PowerShell 版继续保留，选择一种终端，把整条流程做完即可。这里沿用原来的 web-work、kit-04-v12 和个人 p1-portfolio，不另建一套项目。
...
rem clone 下载资源；branch 指定第 4 课，single-branch 只取这一课，depth 1 只取最新提交。
rem 最后的 kit-04-v12 是下载到电脑后的文件夹名称。
git clone --branch p1-guide-04-v1.2 --single-branch --depth 1 "%courseRepo%" kit-04-v12
```

解释：若提示目录已经存在，先查看其 README 核对课次，确认是本课资源才继续。不要删除已有成果或反复 clone。

成功标志：下载成功并返回提示符，目录内出现 kit-04-v12。
...
if exist "kit-04-v12\starter\package.json" (echo READY: starter found) else (echo STOP: starter not found)
```
...
成功标志：web-work 下出现 p1-before-04.zip。已有备份时先核对是本轮导入前的备份，确认后才继续。
...
成功标志：进入 lesson-04。若分支已存在，先核对本课进度，不重新导入。
...
if defined p1Dirty (
  ...
  if not exist "..\p1-before-04.zip" (
    echo STOP: backup is missing.
  ) else (
    if not exist "..\kit-04-v12\starter\package.json" (
      echo STOP: starter is missing.
    ) else (
      rem E 包含子目录，I 指定目标目录，H 保留隐藏项，Y 替换模板同名文件。
      xcopy "..\kit-04-v12\starter\*" "." /E /I /H /Y
    )
  )
)
```
...
在文件资源管理器中解压 p1-before-04.zip，只把自己的展示名、介绍、项目数据、链接和 README 记录迁移回来，具体位置见第 5 步。不要用旧工程整包覆盖新模板。
