# 第06课 Vue作品门户：CMD 全流程实操

从 Git clone 领取模板、修改代码，到提交并推送自己的远程仓库。本版专门用于 Windows CMD。PowerShell 版继续保留，选择一种终端，把整条流程做完即可。这里沿用原来的 web-work、kit-06-v12 和个人 p1-portfolio，不另建一套项目。
...
rem clone 下载资源；branch 指定第 6 课，single-branch 只取这一课，depth 1 只取最新提交。
rem 最后的 kit-06-v12 是下载到电脑后的文件夹名称。
git clone --branch p1-guide-06-v1.2 --single-branch --depth 1 "%courseRepo%" kit-06-v12
```

解释：若提示目录已经存在，先查看其 README 核对课次，确认是本课资源才继续。不要删除已有成果或反复 clone。

成功标志：下载成功并返回提示符，目录内出现 kit-06-v12。
...
if exist "kit-06-v12\starter\package.json" (echo READY: starter found) else (echo STOP: starter not found)
```
...
成功标志：web-work 下出现 p1-before-06.zip。已有备份时先核对是本轮导入前的备份，确认后才继续。
...
成功标志：进入 lesson-06。若分支已存在，先核对本课进度，不重新导入。
...
if defined p1Dirty (
  ...
  if not exist "..\p1-before-06.zip" (
    echo STOP: backup is missing.
  ) else (
    if not exist "..\kit-06-v12\starter\package.json" (
      echo STOP: starter is missing.
    ) else (
      rem E 包含子目录，I 指定目标目录，H 保留隐藏项，Y 替换模板同名文件。
      xcopy "..\kit-06-v12\starter\*" "." /E /I /H /Y
    )
  )
)
```
...
在文件资源管理器中解压 p1-before-06.zip，只把自己的展示名、介绍、项目数据、链接和 README 记录迁移回来，具体位置见第 5 步。不要用旧工程整包覆盖新模板。
...
正常数据迁移到 public/projects.json；备用数组迁移到 src/data/projects.js。个人介绍按组件职责填写 src/components/ProfileHeader.vue、src/App.vue 和 src/components/SiteFooter.vue，先搜索旧展示名定位。 先只迁移内容，再补本课 TODO。

### 5.2 修改 src/components/ProjectCard.vue

打开 p1-portfolio/src/components/ProjectCard.vue。本文件很短，Ctrl+A 选中整个文件。

修改前：

```vue
<script setup>
// TODO-L6: 声明 project 输入和 select 事件。
</script>
...
代码解释：defineProps 接收父组件传来的 project；defineEmits 声明 select 事件；双花括号显示数据；v-if 只在有网址时显示链接；按钮通过 $emit 把项目 id 传回父组件。

按 Ctrl+S 保存，回浏览器 Ctrl+R 刷新，再做下方检查。

### 5.6 逐项观察结果

卡片应显示标题、简介、状态和技能。点击“记录查看”，父组件的记录区域应更新对应作品。切换技能与排序，检查之前功能仍在；切换主题后刷新，确认主题记忆仍在。不要为卡片另写一套数据，数据由父组件传入。两段检查分别回 PPT 第 26、31 页。
