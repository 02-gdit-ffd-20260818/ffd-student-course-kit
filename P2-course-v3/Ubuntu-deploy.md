# Ubuntu部署操作说明（尚未执行服务器部署）

平台：Ubuntu；以下命令在服务器SSH终端逐行执行。前提：服务器已经安装Node.js 24、Git和Nginx，并能正常SSH登录。当前服务器SSH连接被关闭，因此本说明尚未通过远程执行验证。先采用SQLite持久化部署，足以体现真实数据库；不把浏览器存储或静态页面当作数据库。
...
proxy_pass没有结尾斜线，保留/api路径。接口端口默认3000；若被占用，在.env中改PORT，同时修改Nginx上游端口。域名解析完成后再启用HTTPS；登录页面正式开放前应有HTTPS。
