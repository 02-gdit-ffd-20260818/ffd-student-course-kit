...
const article = a => ({ id:Number(a.id),slug:a.slug,title:a.title,summary:a.summary,content:JSON.parse(a.content_json),tags:JSON.parse(a.tags_json),status:a.status,author:a.author,publishedAt:a.published_at })
const user = u => ({ id:Number(u.id),username:u.username,displayName:u.display_name,role:u.role })
export function createCourseApp(db, secret) {
  ...
  for(const method of ['post','put'])app[method](method==='post'?'/api/articles':'/api/articles/:id',admin,async(req,res)=>{
    ...
    const a=toArticleRecord(req.body),values=[a.slug,a.title,a.summary,JSON.stringify(a.content),JSON.stringify(a.tags),a.status,a.author,a.publishedAt]
    const sql=method==='post'?'INSERT INTO course_articles(slug,title,summary,content_json,tags_json,status,author,published_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *':'UPDATE course_articles SET slug=$1,title=$2,summary=$3,content_json=$4,tags_json=$5,status=$6,author=$7,published_at=$8 WHERE id=$9 RETURNING *'
