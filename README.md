...
- CSS 规则格式是 `选择器 { 属性: 值; }`：选择器找到元素，属性说明改什么，值说明改成什么，分号结束一条声明。
- `:root`：网页根元素，可在这里定义全站可用的 CSS 自定义属性。
- `--heading`：自定义属性名，开头两个减号是规定写法；`var(--heading)` 表示读取该值。
- `#254a40`：十六进制颜色，`#` 后六位依次表示红、绿、蓝的强度。
- `px`：CSS 像素，是屏幕布局常用的长度单位；`19px` 表示字号为 19 个 CSS 像素。
- `margin`：边框外部距离，用来隔开相邻元素；`padding`：内容与边框之间的内部距离。
- `padding: 8px 14px`：两个值时，第一个控制上下，第二个控制左右。
- `border-left: 3px solid #b8a27d`：依次表示左边框粗细、实线样式和颜色。
- `background` 是背景色，`color` 是文字颜色，二者含义不同。
- CSS 后写的同等优先级规则通常覆盖前面的规则，因此 `lesson-02-task.css` 必须在 `styles.css` 后加载。
