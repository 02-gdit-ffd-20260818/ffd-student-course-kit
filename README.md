...
第一次操作先看 [Windows鼠标入门_先把第一课运行起来](Windows鼠标入门_先把第一课运行起来.md)，从 VS Code 怎样打开 PowerShell 开始。[全流程手册](实操手册.md) / [Word 版](实操手册.docx)。每个可执行命令框都注明平台、输入位置、当前目录、执行方法，并附 # 注释。初学先使用一个终端。ZIP 作为网络故障时的备用领取方式。
