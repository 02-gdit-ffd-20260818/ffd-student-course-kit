...
无论从哪条路线开始，最终都在独立目录 `p1-lesson-03` 中开发，并推送到本课新仓库 `p1-lesson-03-学号`。

## 路线 A：以上次自己的作品为基础

适合已经完成第 02 课并希望保留个人修改的学生。下面以学生上次仓库 `p1-lesson-02-学号` 为例。
...
`previous` 保留上次仓库来源。不要把本课结果推回 `previous`，这样第 02 课的网站和提交不会被覆盖。
...
1. 在 GitHub 新建空的公开仓库 `p1-lesson-03-学号`，不要勾选 README。
2. 连接本课新仓库：
