# Ubuntu 部署附录：Windows CMD 本机版

配合第六课 PPT 第 45—58 页使用。Windows 用于构建和上传；SSH 登录后，命令才运行在 Ubuntu 24.04 服务器。两者可以共用同一个 VS Code 终端，但必须看清当前提示符。

Windows CMD 提示符通常为 C:\Users\…>，包含盘符，不带 PS。Ubuntu 通常显示 账号@主机名:~$。下方每一步都标注当前平台，按顺序一次执行一个代码框。Windows CMD 用 rem 注释，Ubuntu Bash 用 # 注释；按代码框的平台选择，均可连同注释一起复制。发生错误先停止后续依赖步骤。
...
打开方式：Windows 上打开 VS Code 的 p1-portfolio，点击“终端 → 新建终端”，选择 Command Prompt（命令提示符）。如果网页服务还在运行，先 Ctrl+C 停止，再输入以下定位命令。

运行平台：Windows CMD。输入位置：VS Code 下方终端，起始目录任意。运行方法：粘贴本代码框并按 Enter。
...
mkdir -p ~/p1-upload
# 结束本次 SSH 连接，回到原来的 Windows CMD。
exit
```

解释：建立用户上传目录；exit结束SSH，返回Windows终端。

成功标志：安装成功，返回 Windows CMD 的盘符路径提示符。
...
解释：-r递归复制dist到远程用户的p1-upload目录；CMD 变量用两侧百分号展开，冒号后是服务器目录。

成功标志：SCP传输完成，没有错误。

遇到问题：serverTarget为空：重新用 set /p 输入；上传的是dist而非node_modules。
...
Windows 部分用 CMD 的 rem、百分号变量和 cd /d。执行 ssh 并成功登录后，Ubuntu 部分仍使用 Bash 与 sudo，不把 Windows 的 rem 复制到 Ubuntu。执行 exit 才回到原 CMD。
