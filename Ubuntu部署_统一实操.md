# Ubuntu 部署附录：Windows 双终端统一入口

Windows 框二选一；SSH 登录后的 Ubuntu Bash 框两种入口共用。每个阶段核对运行平台，exit 后回到原 Windows 终端。

配合第六课 PPT 第 45—58 页使用。Windows 用于构建和上传；SSH 登录后，命令才运行在 Ubuntu 24.04 服务器。两者可以共用同一个 VS Code 终端，但必须看清当前提示符。

Windows CMD 提示符通常为 C:\Users\…>；PowerShell 通常以 PS 开头。Ubuntu 通常显示 账号@主机名:~$。下方每一步都标注当前平台，按顺序一次执行一个代码框。Windows CMD 用 rem 注释，PowerShell 和 Ubuntu Bash 用 # 注释；按代码框的平台选择，均可连同注释一起复制。发生错误先停止后续依赖步骤。
...
打开方式：Windows 上打开 VS Code 的 p1-portfolio，点击“终端 → 新建终端”，选择本课一直使用的 Command Prompt 或 PowerShell。如果网页服务还在运行，先 Ctrl+C 停止，再输入以下定位命令。

运行平台：Windows CMD 或 PowerShell。输入位置：VS Code 下方终端，起始目录任意。运行方法：粘贴本代码框并按 Enter。
...
P1 主课已完成，以下配合第六课 PPT 第 45—58 页按需实施。
...
运行平台：Windows 10/11 · CMD 或 PowerShell（本机，二选一）。

运行位置：VS Code 下方同一个终端；当前应显示 Windows 的盘符路径，目录为自己的 p1-portfolio。
...
```bat
rem 输入老师提供的账号和服务器地址；执行后先回答问题，再进入下一框。
set "serverTarget="
rem 出现问题后输入账号和地址并回车，不能把下一条命令当作答案。
set /p "serverTarget=输入账号@服务器地址: "
```
...
```bat
rem 连接该服务器；认证成功后，同一终端中的命令将运行在 Ubuntu。
ssh "%serverTarget%"
```
...
mkdir -p ~/p1-upload
# 结束本次 SSH 连接，回到原来的 Windows 终端。
exit
```

解释：建立用户上传目录；exit结束SSH，返回Windows终端。

成功标志：安装成功，返回 Windows 的盘符路径提示符。
...
```bat
rem 重新构建 Windows 工程的部署文件。
npm.cmd run build
rem READY 表示构建入口存在，STOP 时不要继续上传。
...
```bat
rem 递归上传 dist 到服务器账号家目录下的 p1-upload。
...
解释：-r递归复制dist到远程用户的p1-upload目录；CMD 使用百分号变量；PowerShell 使用花括号分隔变量与冒号。冒号后是服务器目录。

成功标志：SCP传输完成，没有错误。

遇到问题：serverTarget为空：重新用所选终端的输入框填写；上传的是dist而非node_modules。
...
```bat
rem 上传第六课提供的 Nginx 配置。
scp "..\kit-06-v12\deploy\nginx-p1.conf" "%serverTarget%:p1-upload/"
```
...
解释：上传本课提供的配置文件。本版已写成单行，无需续行；此文件只服务本练习站点。
...
Windows 部分按框内标签选择 CMD 或 PowerShell。执行 ssh 并成功登录后，Ubuntu 部分仍使用 Bash 与 sudo，不把 Windows 的 rem 复制到 Ubuntu。执行 exit 才回到原 Windows 终端。
