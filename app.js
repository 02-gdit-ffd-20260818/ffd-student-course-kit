...
toTopButton.addEventListener('click', () => {
  // TODO 01：在这里加一行，让页面滚回顶部
  //   要加的那一行：window.scrollTo({ top: 0, behavior: 'smooth' })
...
function handleScroll() {
  ...
  // TODO 02：把下面这一行的第二个参数补完整。
  //   现在写死成 true，所以按钮一直显示。
  //   要改成：window.scrollY > window.innerHeight * 0.6
  toTopButton.classList.toggle('is-visible', true) // ← 改这一行
...
function updateProgress() {
  ...
  // TODO 03：把下面这一行等号右边补完整。
  //   现在写死成 '0%'，所以进度条永远看不见。
  //   要改成：`${ratio * 100}%`      注意是反引号 ` 不是单引号 '
  progressBar.style.width = '0%' // ← 改这一行
}
...
function showCurrent(hash) {
  ...
  // TODO 04 第一行：把下面这一行等号右边补完整。
  //   现在写死成空字符串，所以右下角徽标永远是空的。
  //   要改成：sectionNames.get(current) || ''
  indicator.textContent = '' // ← 改这一行
  ...
  navLinks.forEach(link => {
    ...
    // TODO 04 第二行：把下面这一行的第二个参数补完整。
    //   现在写死成 false，所以导航从来不高亮。
    //   要改成：isCurrent
    link.classList.toggle('is-current', false) // ← 改这一行
...
// TODO 05：在这里加两行。
//   第一行：window.addEventListener('hashchange', () => showCurrent(location.hash))
//   第二行：showCurrent(location.hash)
//           ↑ 这一行最容易漏。缺了它，别人把 index.html#skills 发给你，
//             你打开时什么都不高亮。
