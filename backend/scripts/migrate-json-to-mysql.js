...
const run = async () => {
  ...
  if (userIds.length !== users.length || new Set(userIds).size !== userIds.length)
    throw new Error('JSON 数据中存在缺失或重复的用户 ID。')
  ...
  if (users.some((user) => typeof user.password !== 'string' || !user.password))
    throw new Error('JSON 数据中存在缺失的用户密码。')
  if (new Set(emails).size !== emails.length) throw new Error('JSON 数据中存在重复邮箱；请先清理后再迁移。')
  if (users.some((user) => user.classId && !classIds.has(user.classId)))
    throw new Error('JSON 数据中存在引用不存在班级的用户。')
  const admins = users.filter(
    (user) =>
      String(user.email || '')
        .trim()
        .toLowerCase() === 'admin@system.com',
  )
  if (admins.length !== 1) throw new Error('JSON 数据必须且只能包含一个 admin@system.com 管理员。')
  if (
    admins[0].password === LEGACY_ADMIN_HASH &&
    String(process.env.ADMIN_INITIAL_PASSWORD || '').length < 8
  ) {
  ...
  await withTransaction(async (connection) => {
    ...
    for (const item of classes) {
      await connection.execute(
        'INSERT INTO classes (id, name, description, teacher, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
        [
          item.id,
          item.name,
          item.description || '',
          item.teacher || '',
          dateOrNow(item.createdAt),
          item.updatedAt ? dateOrNow(item.updatedAt) : null,
        ],
      )
    }
    for (const user of users) {
      ...
      await connection.execute(
        ...
        [
          ...
          email,
          await passwordFor(user, email),
          user.avatar || '',
          role,
          classId,
          JSON.stringify(profileOf(user)),
          user.avatarIndex ?? null,
          dateOrNow(user.createdAt),
          user.updatedAt ? dateOrNow(user.updatedAt) : null,
        ],
      )
    }
    for (const group of groups)
      await connection.execute(
        'INSERT INTO synonym_groups (id, name, category, synonyms, updated_at) VALUES (?, ?, ?, ?, ?)',
        [String(group.id), group.name, group.category, JSON.stringify(group.synonyms || []), new Date()],
      )
    for (const hobby of hobbies)
      await connection.execute('INSERT INTO standard_hobbies (id, name, category) VALUES (?, ?, ?)', [
        hobby.id,
        hobby.name,
        hobby.category || '其他',
      ])
  ...
  console.log(
    `迁移完成：${classes.length} 个班级、${users.length} 位用户、${groups.length} 个同义词组、${hobbies.length} 条标准爱好。`,
  )
  await pool.end()
}
