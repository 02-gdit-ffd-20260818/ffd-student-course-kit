...
日期：2026-09-18。每次3×45分钟。CMD与PowerShell合并，选择一种终端。功能基础完整，主文件：`src/components/ArticleCard.vue`。
...
摘要来自 article.summary；详情使用 slug 而非 id
...
领取目录：next-kit-v2/starter/lesson-07
个人工程：web-work/p2-work
重点文件：src/components/ArticleCard.vue
学生指南：next-kit-v2/docs/lesson-07.md
完整功能基础已提供，重点完成两项任务。
...
cd web-work
git clone --branch p2p5-20260918-v2.0 --depth 1 https://github.com/02-gdit-ffd-20260818/ffd-student-course-kit.git next-kit-v2
if not exist p2-work xcopy "next-kit-v2\starter\lesson-07" "p2-work\" /E /I /H
cd p2-work
```
...
git clone --branch p2p5-20260918-v2.0 --depth 1 https://github.com/02-gdit-ffd-20260818/ffd-student-course-kit.git next-kit-v2
if (!(Test-Path 'p2-work')) { Copy-Item 'next-kit-v2/starter/lesson-07' 'p2-work' -Recurse }
Set-Location 'p2-work'
```
...
## PPT 第10页：任务 A：补上摘要显示

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p2-work**。

打开 src/components/ArticleCard.vue
搜索 TODO-A 或任务 A 的注释。
找到占位实现，修改其对应内容。
保存，运行下面的成功检查。
...
运行平台：**VS Code 文件编辑区**。当前位置：**src/components/ArticleCard.vue**。
...
## PPT 第13页：任务 B：补上阅读全文事件

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p2-work**。

继续编辑 src/components/ArticleCard.vue
搜索 TODO-B 或任务 B 的注释。
替换占位实现；保留任务 A 的成果。
做一个自己的内容或规则变式。
...
成功标志：摘要来自 article.summary；详情使用 slug 而非 id
...
git diff
git add .
git commit -m "第07课：补上摘要显示与补上阅读全文事件"
git push
```
...
本课完整学生工程：`starter/lesson-07`；教师完整答案在教师包。差异文件只列今天两处变化。学习者只补任务对应实现，其他模块逐步调用与理解。
