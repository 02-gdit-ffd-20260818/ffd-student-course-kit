...
日期：2026-09-18。每次3×45分钟。CMD与PowerShell合并，选择一种终端。功能基础完整，主文件：`server/services/articleInput.js`。
...
## PPT 第1页：第09课 · 博客接口与请求

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p2-work**。

无标题 POST 返回400；标签去空白并去重
...
领取目录：next-kit-v2/starter/lesson-09
个人工程：web-work/p2-work
重点文件：server/services/articleInput.js
学生指南：next-kit-v2/docs/lesson-09.md
完整功能基础已提供，重点完成两项任务。
...
git status
git apply --check "..\next-kit-v2\upgrades\lesson-09.patch"
git apply "..\next-kit-v2\upgrades\lesson-09.patch"
```
...
git status
git apply --check '../next-kit-v2/upgrades/lesson-09.patch'
git apply '../next-kit-v2/upgrades/lesson-09.patch'
```
...
## PPT 第10页：任务 A：在服务器拦截空标题

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p2-work**。

打开 server/services/articleInput.js
搜索 TODO-A 或任务 A 的注释。
找到占位实现，修改其对应内容。
保存，运行下面的成功检查。
...
运行平台：**VS Code 文件编辑区**。当前位置：**server/services/articleInput.js**。
...
## PPT 第13页：任务 B：规范化标签数组

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p2-work**。

继续编辑 server/services/articleInput.js
搜索 TODO-B 或任务 B 的注释。
替换占位实现；保留任务 A 的成果。
做一个自己的内容或规则变式。
...
git diff
git add .
git commit -m "第09课：在服务器拦截空标题与规范化标签数组"
git push
```
...
本课完整学生工程：`starter/lesson-09`；教师完整答案在教师包。差异文件只列今天两处变化。学习者只补任务对应实现，其他模块逐步调用与理解。
