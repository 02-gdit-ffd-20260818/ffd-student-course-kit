...
日期：2026-09-18。每次3×45分钟。CMD与PowerShell合并，选择一种终端。功能基础完整，主文件：`database/classroom-query.mjs`。
...
## PPT 第1页：第10课 · 博客数据库与备份

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p2-work**。

查询只列已发布文章；分组统计与实际数据一致
...
领取目录：next-kit-v2/starter/lesson-10
个人工程：web-work/p2-work
重点文件：database/classroom-query.mjs
学生指南：next-kit-v2/docs/lesson-10.md
完整功能基础已提供，重点完成两项任务。
...
cd /d "%USERPROFILE%\web-work\p2-work"
git status
git apply --check "..\next-kit-v2\upgrades\lesson-10.patch"
git apply "..\next-kit-v2\upgrades\lesson-10.patch"
```
...
Set-Location -LiteralPath (Join-Path $env:USERPROFILE 'web-work/p2-work')
git status
git apply --check '../next-kit-v2/upgrades/lesson-10.patch'
git apply '../next-kit-v2/upgrades/lesson-10.patch'
```
...
运行平台：**Windows CMD / PowerShell 通用**。当前位置：**自己的工程 p2-work**。
...
## PPT 第8页：终端 B：配置并启动 API

运行平台：**Windows CMD / PowerShell 通用**。当前位置：**自己的工程 p2-work**。

终端菜单 → 新建终端，确认仍在个人工程。prepare-env 生成本机配置并保留已有文件。
...
成功标志：API 启动，另开浏览器访问 http://127.0.0.1:3000/health

排错：A 是网页、B 是接口；不要在 A 正运行时粘贴 Git 命令。
...
## PPT 第10页：任务 A：查询已发布文章

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p2-work**。

打开 database/classroom-query.mjs
搜索 TODO-A 或任务 A 的注释。
找到占位实现，修改其对应内容。
保存，运行下面的成功检查。
...
运行平台：**VS Code 文件编辑区**。当前位置：**database/classroom-query.mjs**。

这是修改位置的准确代码；替换占位实现，保留周围结构。

```text
SELECT id, title, status FROM articles WHERE status = 'published' ORDER BY id
```
...
## PPT 第13页：任务 B：按状态统计文章

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p2-work**。

继续编辑 database/classroom-query.mjs
搜索 TODO-B 或任务 B 的注释。
替换占位实现；保留任务 A 的成果。
做一个自己的内容或规则变式。
...
git diff
git add .
git commit -m "第10课：查询已发布文章与按状态统计文章"
git push
```
...
本课完整学生工程：`starter/lesson-10`；教师完整答案在教师包。差异文件只列今天两处变化。学习者只补任务对应实现，其他模块逐步调用与理解。

## 数据库实操：务必按顺序

终端C，Windows CMD/PowerShell通用，位置p2-work：
```text
npm.cmd run db:migrate
npm.cmd run db:seed
node database/classroom-query.mjs
npm.cmd run db:verify
npm.cmd run db:backup -- ./var/backups/lesson10.sqlite
```

migrate建立/更新表；seed插入演示资料，不覆盖已有同ID；query执行任务SQL；verify检查完整性和外键；backup将当前库复制到明确备份文件。结果应有已发布文章和状态数量。SQL写在query.mjs引号里，不输入Windows终端。数据库文件在var目录，不上传Git。

恢复演练使用单独练习库，不覆盖正在使用的数据库。先在B按Ctrl+C停止API。CMD：
```bat
rem 恢复到独立练习文件，不影响主库。
set "DATABASE_PATH=./var/restore-practice.sqlite"
npm.cmd run db:restore -- ./var/backups/lesson10.sqlite
npm.cmd run db:verify
set "DATABASE_PATH="
```
...
```powershell
# 恢复练习后清除本终端临时环境变量。
$env:DATABASE_PATH = './var/restore-practice.sqlite'
npm.cmd run db:restore -- ./var/backups/lesson10.sqlite
npm.cmd run db:verify
Remove-Item Env:DATABASE_PATH
```

DATABASE_PATH只在当前终端指定数据库位置；清除后继续默认主库。重新在B启动API，用页面检查主库数据仍在。MySQL属于可选扩展，见原工程database/mysql，不要求本课安装两套数据库。
