...
日期：2026-09-18。每次3×45分钟。CMD与PowerShell合并，选择一种终端。功能基础完整，主文件：`server/services/auth.js`。
...
## PPT 第1页：第11课 · 博客登录与生产部署

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p2-work**。

正确登录能写入；匿名401、越权403；重启数据仍在
...
领取目录：next-kit-v2/starter/lesson-11
个人工程：web-work/p2-work
重点文件：server/services/auth.js
学生指南：next-kit-v2/docs/lesson-11.md
完整功能基础已提供，重点完成两项任务。
...
git status
git apply --check "..\next-kit-v2\upgrades\lesson-11.patch"
git apply "..\next-kit-v2\upgrades\lesson-11.patch"
```
...
git status
git apply --check '../next-kit-v2/upgrades/lesson-11.patch'
git apply '../next-kit-v2/upgrades/lesson-11.patch'
```
...
## PPT 第10页：任务 A：恢复有效令牌判断

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p2-work**。

打开 server/services/auth.js
搜索 TODO-A 或任务 A 的注释。
找到占位实现，修改其对应内容。
保存，运行下面的成功检查。
...
运行平台：**VS Code 文件编辑区**。当前位置：**server/services/auth.js**。
...
## PPT 第13页：任务 B：提取 Bearer 令牌

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p2-work**。

继续编辑 server/services/auth.js
搜索 TODO-B 或任务 B 的注释。
替换占位实现；保留任务 A 的成果。
做一个自己的内容或规则变式。
...
成功标志：正确登录能写入；匿名401、越权403；重启数据仍在
...
git diff
git add .
git commit -m "第11课：恢复有效令牌判断与提取 Bearer 令牌"
git push
```
...
本课完整学生工程：`starter/lesson-11`；教师完整答案在教师包。差异文件只列今天两处变化。学习者只补任务对应实现，其他模块逐步调用与理解。

## 登录、权限、持久化验证

node tools/prepare-env.mjs后，VS Code文件列表找到.env（在工程根目录），查看ADMIN_USERNAME、ADMIN_PASSWORD。启动API后，用管理页面登录。密码只在本机.env中查看；本文件和var目录不提交。SESSION_SECRET随机生成至少32字符，保证.env是本机文件。

匿名打开阅读正常；不登录写操作返回401；普通角色不能删除，返回403。服务停掉再启动后文章仍在SQLite。完整权限判断与加密工具由工程提供，课堂解释认证、授权的区别并追踪一次请求。

部署步骤见项目《Ubuntu部署实操.md》，前端dist和API一起部署，地址不依赖教师服务器。
