...
日期：2026-09-18。每次3×45分钟。CMD与PowerShell合并，选择一种终端。功能基础完整，主文件：`src/domain/member.js`。
...
领取目录：next-kit-v2/starter/lesson-12
个人工程：web-work/p3-work
重点文件：src/domain/member.js
学生指南：next-kit-v2/docs/lesson-12.md
完整功能基础已提供，重点完成两项任务。
...
cd web-work
if not exist p3-work xcopy "next-kit-v2\starter\lesson-12" "p3-work\" /E /I /H
cd p3-work
```
...
if (!(Test-Path 'p3-work')) { Copy-Item 'next-kit-v2/starter/lesson-12' 'p3-work' -Recurse }
Set-Location 'p3-work'
```
...
## PPT 第10页：任务 A：完成技能筛选

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p3-work**。

打开 src/domain/member.js
搜索 TODO-A 或任务 A 的注释。
找到占位实现，修改其对应内容。
保存，运行下面的成功检查。
...
运行平台：**VS Code 文件编辑区**。当前位置：**src/domain/member.js**。
...
## PPT 第13页：任务 B：累加技能人数

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p3-work**。

继续编辑 src/domain/member.js
搜索 TODO-B 或任务 B 的注释。
替换占位实现；保留任务 A 的成果。
做一个自己的内容或规则变式。
...
git diff
git add .
git commit -m "第12课：完成技能筛选与累加技能人数"
git push
```
...
本课完整学生工程：`starter/lesson-12`；教师完整答案在教师包。差异文件只列今天两处变化。学习者只补任务对应实现，其他模块逐步调用与理解。
