...
日期：2026-09-18。每次3×45分钟。CMD与PowerShell合并，选择一种终端。功能基础完整，主文件：`server/workflow.js`。
...
## PPT 第1页：第13课 · 社区提交与审核

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p3-work**。

提交后审核可通过；普通成员不能审核；重复审批拒绝
...
领取目录：next-kit-v2/starter/lesson-13
个人工程：web-work/p3-work
重点文件：server/workflow.js
学生指南：next-kit-v2/docs/lesson-13.md
完整功能基础已提供，重点完成两项任务。
...
cd /d "%USERPROFILE%\web-work\p3-work"
git status
git apply --check "..\next-kit-v2\upgrades\lesson-13.patch"
git apply "..\next-kit-v2\upgrades\lesson-13.patch"
```
...
Set-Location -LiteralPath (Join-Path $env:USERPROFILE 'web-work/p3-work')
git status
git apply --check '../next-kit-v2/upgrades/lesson-13.patch'
git apply '../next-kit-v2/upgrades/lesson-13.patch'
```
...
运行平台：**Windows CMD / PowerShell 通用**。当前位置：**自己的工程 p3-work**。
...
成功标志：API 启动，另开浏览器访问 http://127.0.0.1:3020/health
...
## PPT 第10页：任务 A：允许提交后的审核状态

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p3-work**。

打开 server/workflow.js
搜索 TODO-A 或任务 A 的注释。
找到占位实现，修改其对应内容。
保存，运行下面的成功检查。
...
运行平台：**VS Code 文件编辑区**。当前位置：**server/workflow.js**。
...
## PPT 第13页：任务 B：校验审核者角色

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p3-work**。

继续编辑 server/workflow.js
搜索 TODO-B 或任务 B 的注释。
替换占位实现；保留任务 A 的成果。
做一个自己的内容或规则变式。
...
git diff
git add .
git commit -m "第13课：允许提交后的审核状态与校验审核者角色"
git push
```
...
本课完整学生工程：`starter/lesson-13`；教师完整答案在教师包。差异文件只列今天两处变化。学习者只补任务对应实现，其他模块逐步调用与理解。

## 协同主路径

打开本机.env，分别查看MEMBER_USERNAME/MEMBER_PASSWORD与REVIEWER_USERNAME/REVIEWER_PASSWORD。用成员账号提交一份虚构资料→退出→审核账号登录→审核通过→刷新公开列表。普通成员不能审核；重复审核必须拒绝。CSV先预览再导入是选做，完整基础代码保留。
