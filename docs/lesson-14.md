...
日期：2026-09-18。每次3×45分钟。CMD与PowerShell合并，选择一种终端。功能基础完整，主文件：`src/services/share.js`。
...
## PPT 第1页：第14课 · 祝福卡片与可靠生成

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p4-work**。

切换主题后分享能恢复；损坏分享不崩溃；无AI密钥仍生成
...
领取目录：next-kit-v2/starter/lesson-14
个人工程：web-work/p4-work
重点文件：src/services/share.js
学生指南：next-kit-v2/docs/lesson-14.md
完整功能基础已提供，重点完成两项任务。
...
cd web-work
if not exist p4-work xcopy "next-kit-v2\starter\lesson-14" "p4-work\" /E /I /H
cd p4-work
```
...
if (!(Test-Path 'p4-work')) { Copy-Item 'next-kit-v2/starter/lesson-14' 'p4-work' -Recurse }
Set-Location 'p4-work'
```
...
运行平台：**Windows CMD / PowerShell 通用**。当前位置：**自己的工程 p4-work**。
...
成功标志：API 启动，另开浏览器访问 http://127.0.0.1:3030/health
...
## PPT 第10页：任务 A：保留分享主题

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p4-work**。

打开 src/services/share.js
搜索 TODO-A 或任务 A 的注释。
找到占位实现，修改其对应内容。
保存，运行下面的成功检查。
...
运行平台：**VS Code 文件编辑区**。当前位置：**src/services/share.js**。
...
## PPT 第13页：任务 B：校验并恢复分享内容

运行平台：**浏览器 / VS Code**。当前位置：**自己的工程 p4-work**。

继续编辑 src/services/share.js
搜索 TODO-B 或任务 B 的注释。
替换占位实现；保留任务 A 的成果。
做一个自己的内容或规则变式。
...
成功标志：切换主题后分享能恢复；损坏分享不崩溃；无AI密钥仍生成
...
git diff
git add .
git commit -m "第14课：保留分享主题与校验并恢复分享内容"
git push
```
...
本课完整学生工程：`starter/lesson-14`；教师完整答案在教师包。差异文件只列今天两处变化。学习者只补任务对应实现，其他模块逐步调用与理解。

## 离线保底与AI选做

AI_PROVIDER=fallback时无外部账号和密钥也能完成生成、主题与分享主路径。真实模型调用仅由教师选做配置，密钥留在服务器环境变量，不能放VITE_变量或学生前端。先完成无外部依赖的本课两项任务，再演示超时回退。分享网址可能包含祝福正文，课堂只用虚构内容。
