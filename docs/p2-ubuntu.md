...
平台：Ubuntu Bash，位置~/classroom-apps/p2。
...
serviceUser="$(id -un)"
# 建立本项目专用数据、备份和网页发布目录。
sudo install -d -o "$serviceUser" -g "$(id -gn)" /srv/classroom/p2/data /srv/classroom/p2/backups /srv/classroom/p2/releases
# 仅数据库项目需要把.env中的DATABASE_PATH改为下面持久路径。
```

在nano .env中将DATABASE_PATH改为`/srv/classroom/p2/data/p2-blog.sqlite`，DB_DRIVER保持sqlite。
...
平台：Ubuntu Bash，位置~/classroom-apps/p2。沿用第4节的appDirectory/serviceUser；重新登录需要重新赋值。

```bash
# 写入本项目独立服务，不更改其他项目服务。
sudo tee /etc/systemd/system/classroom-p2.service >/dev/null <<EOF
[Unit]
Description=Classroom P2 API
...
sudo systemctl enable --now classroom-p2
sudo systemctl status classroom-p2 --no-pager
curl -f http://127.0.0.1:3000/health
```

tee写文件；daemon-reload重新读取；enable --now设置开机启动并立即运行；status看状态；curl核对API。失败先journalctl -u classroom-p2 -n 40 --no-pager，看第一条错误，修复.env或目录后restart。
...
cp -a dist "/srv/classroom/p2/releases/$releaseName"
ln -sfnT "/srv/classroom/p2/releases/$releaseName" /srv/classroom/p2/current
# 本项目在独立教学端口运行，API同源转发。
sudo tee /etc/nginx/conf.d/classroom-p2.conf >/dev/null <<'EOF'
server {
    listen 8080;
    server_name _;
    root /srv/classroom/p2/current;
    index index.html;
...
先在Windows完成测试、commit、push。Ubuntu cd ~/classroom-apps/p2→git status确认干净→git pull --ff-only origin main（仅允许快进更新）→npm ci→npm test→npm run build→重复第6节网页发布→sudo systemctl restart classroom-p2→curl health。不覆盖.env和持久数据目录。
...
恢复网页：先ls /srv/classroom/p2/releases查看实际存在版本，再用ln -sfnT指向确实存在的旧网页目录。恢复API：git switch --detach 自己已发布的真实标签→npm ci→sudo systemctl restart classroom-p2；恢复后检查health与业务。继续开发前git switch main。涉及数据库迁移时，网页回退不等于数据库自动回退，需按真实备份恢复并确认模式兼容。

本机日志：journalctl -u classroom-p2 -n 40 --no-pager；Nginx日志在/var/log/nginx/。记录部署时间、Git提交、网页版本目录、数据库备份位置和健康检查结果。exit退出SSH返回Windows。
