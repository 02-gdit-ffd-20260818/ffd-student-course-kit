...
平台：Ubuntu Bash，任意目录。clone的是自己的公开工程，已经完成第14课并push；不是教师资源仓库。
...
卡片不需要数据库，AI_PROVIDER保持fallback，无外部账号仍可运行。
...
另一Ubuntu终端执行curl -f http://127.0.0.1:3030/health，结果ok=true。回API终端Ctrl+C停止，继续下面自动服务配置。
...
cp复制dist；ln切换当前网页；Nginx的/api转发到本机Express，其他路由返回Vue首页。Windows浏览器打开http://自己的服务器地址:8082/。若本机curl成功公网打不开，教师核对云安全组的对应教学端口。每个项目端口不同，可在同一服务器保存全部项目。
