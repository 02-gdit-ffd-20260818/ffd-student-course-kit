...
export const useUserStore = defineStore('user', () => {
  ...
  const adminCredentials = () => ({
    adminEmail: user.value?.email,
    // 本教学项目沿用既有的管理员校验方式；生产环境应改为登录令牌。
    adminPassword: 'admin123'
  })

  const createUserByAdmin = async (userData) => {
    const response = await userService.createUserByAdmin(userData, adminCredentials())
    await fetchOtherUsers()
    return response
  }

  const resetUserPassword = async (email) => {
    const response = await userService.resetUserPasswordByAdmin(email, adminCredentials())
    return response
  }
