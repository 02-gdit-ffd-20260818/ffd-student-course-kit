...
const configPath=resolve(process.env.CONFIG_ENV_PATH||'.env')
if(existsSync(configPath))copyFileSync(configPath,join(bundle,'.env'))
console.log('4/4 媒体与配置已复制')
console.log('完整备份：',basename(bundle),'；数据库与WAL逐字节SHA-256校验：通过')
console.log(`备份目录：${bundle}；包含数据库、上传媒体及可用的环境配置，不提交Git。`)
