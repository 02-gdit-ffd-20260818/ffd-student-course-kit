...
// ============ TODO 03（简单）：上线之后要逐条验 ============
// **部署成功不等于功能正常。** 每次上线都手点一遍容易漏，
// 所以写一个脚本把该验的都验一遍。
//
// 终端里看得到的结果：npm run verify:live 一条条打勾，
// 全过才算上线成功；有一条不过就说明线上有问题。
//
// TODO：补全这一行，对 /health 做四项断言：
//   状态码 200
//   body.ok 是 true
//   body.database 是 'SQLite'（确认连的是数据库，不是走了兜底）
//   body.project 是 'P2内容发布平台v4.0'（确认部署的是这个项目，没发错）
//
// 想一想：为什么线上验证不能只看"首页能打开"？
// 因为首页是静态文件，它能打开只说明 Nginx 活着，
// **完全不能说明数据库连上了、接口没报错**。
// /health 会真的执行一次 SELECT 1，所以它能同时验证 Express 和 SQLite。
// ==================================================
