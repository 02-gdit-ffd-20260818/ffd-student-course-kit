...
export function createBlogApp(db, secret, options={}) {
  ...
  function admin(req,res,next) { if(!req.user)return fail(res,401,'请先登录');if(req.user.role!=='admin')return fail(res,403,'仅管理员可管理文章');next() }
  // 上传文件需要可持久化磁盘，因此只在 SQLite 服务器环境开放。
  if(db.driver==='SQLite'){
  ...
  app.get('/health',async(req,res)=>{await db.query('SELECT 1');res.json({ok:true,database:db.driver,project:'P2内容发布平台v4.0'})})
  ...
  app.post('/api/auth/register',async(req,res)=>{
    ...
    if((await db.query('SELECT id FROM users WHERE username=$1',[username])).length)return fail(res,409,'用户名已被使用')
    const {salt,hash}=hashPassword(password)
    const [u]=await db.query("INSERT INTO users(username,display_name,password_salt,password_hash) VALUES($1,$2,$3,$4) RETURNING *",[username,displayName,salt,hash])
  ...
  app.get('/api/articles',async(req,res)=>{
    const rows=await db.query(req.user?.role==='admin'?'SELECT * FROM articles ORDER BY id':"SELECT * FROM articles WHERE status='published' ORDER BY id")
  ...
  for(const method of ['post','put'])app[method](method==='post'?'/api/articles':'/api/articles/:id',admin,async(req,res)=>{
    ...
    const sql=method==='post'?'INSERT INTO articles(slug,title,summary,content_json,tags_json,status,author,published_at,media_json) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *':'UPDATE articles SET slug=$1,title=$2,summary=$3,content_json=$4,tags_json=$5,status=$6,author=$7,published_at=$8,media_json=$9 WHERE id=$10 RETURNING *'
  ...
  app.delete('/api/articles/:id',admin,async(req,res)=>{const rows=await db.query('DELETE FROM articles WHERE id=$1 RETURNING id',[Number(req.params.id)]);if(!rows.length)return fail(res,404,'文章不存在');res.sendStatus(204)})
  async function published(id){return (await db.query("SELECT id FROM articles WHERE id=$1 AND status='published'",[Number(id)])).length>0}
  ...
  app.get('/api/articles/:id/comments',async(req,res)=>{
    ...
    const rows=await db.query('SELECT c.id,c.user_id,c.body,c.created_at,u.display_name FROM comments c JOIN users u ON u.id=c.user_id WHERE c.article_id=$1 ORDER BY c.id DESC LIMIT 200',[Number(req.params.id)])
  ...
  app.post('/api/articles/:id/comments',signed,async(req,res)=>{
    ...
    const count=await db.query('SELECT COUNT(*) AS total FROM comments WHERE user_id=$1',[req.user.sub]);if(Number(count[0].total)>=200)return fail(res,429,'评论数量已达体验上限')
    const [row]=await db.query('INSERT INTO comments(article_id,user_id,body) VALUES($1,$2,$3) RETURNING id',[Number(req.params.id),req.user.sub,body]);res.status(201).json({data:{id:Number(row.id)}})
  })
  ...
  app.delete('/api/comments/:id',signed,async(req,res)=>{
    ...
    await db.query('DELETE FROM comments WHERE id=$1',[Number(req.params.id)]);res.sendStatus(204)
  })
