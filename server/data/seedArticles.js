export const seedArticles = [
  {
    ...
    "content": [
      "本次迭代目标：把一页博客拆成页面框架、文章列表、文章卡片和文章详情。先让页面可读，再讨论抽象；每个组件只承担一个明确任务。",
  ...
  },
  {
    ...
    "content": [
      "本次迭代目标：列表中的每篇文章都有独立地址，用户可以直接打开、刷新或分享该地址。",
  ...
  },
  {
    ...
    "content": [
      "本次迭代目标：管理员填写文章表单后，浏览器把数据提交给后端；列表重新读取保存结果。页面显示成功不等于数据库已经保存。",
      "打开 src/views/ArticleFormView.vue，观察 v-model、submit.prevent 和字段错误。标题、摘要、正文、标签分别收集，发送前检查必填内容。",
      "HTTP 分工：GET /api/articles 查询；POST /api/articles 新增；PUT /api/articles/:id 更新；DELETE /api/articles/:id 删除。写入接口要求管理员身份。",
  ...
  },
  {
    ...
    "content": [
      "本次迭代目标：浏览器关闭后数据依然存在。系统使用真实 SQLite 文件，后端负责读写，前端不能直接接触数据库密码。",
      "数据结构：users 保存账号和密码摘要；articles 保存文章；comments 保存 article_id、user_id 和评论正文。一篇文章可以有多条评论，一个用户可以发表多条评论。",
      "查询示例：\nSELECT c.body, u.display_name\nFROM comments c\nJOIN users u ON u.id = c.user_id\nWHERE c.article_id = ?;\n问号的位置由驱动绑定参数，用户输入不拼接到 SQL 字符串。",
      "打开 server/blog-db.js，找到 CREATE TABLE 和 REFERENCES。删除文章时 ON DELETE CASCADE 清理所属评论；用户名 UNIQUE 防止重复注册。数据库文件位于 var/blog.sqlite，不提交到 Git。",
  ...
  },
  {
    ...
    "summary": "密码摘要、令牌与权限",
    "content": [
      "本次迭代目标：访客可以阅读文章和评论；注册登录后可以留言；用户只删除自己的留言；管理员可以管理文章与评论。",
  ...
  },
  {
    ...
    "content": [
      "本次迭代目标：同学能够在自己的仓库中留下完整修改记录，并通过网址向别人展示成果。",
  ...
  },
  {
    ...
    "content": [
      "本次迭代目标：先用一张图解释“浏览器→接口→数据库”，再添加音频和视频。媒体使用结构化字段，不把任意HTML粘贴进正文。下面的示例文件随工程提供，可以在本机直接播放。",
      ...
      "操作步骤：管理员登录→管理→新建或编辑文章→添加媒体→选择类型→填写/media/路径或HTTPS资源链接，或上传本机文件→填写说明及图片替代文字→指定段落序号→保存→打开详情验收。上传文件最多40MB，使用MP4/H.264视频便于浏览器播放。",
      "工程知识：浏览器把媒体作为独立HTTP请求获取，数据库保存的是媒体地址和说明；上传文件保存在var/media，图片、音频和视频本身不会塞进评论表。升级文章表增加media_json字段时，保留已有文章、用户及评论。",
      "验收：图片不变形；音频和视频可主动播放、暂停、拖动；刷新仍有媒体；错误资源给出失败提示；手机宽度不溢出；普通用户不能上传文件。基础版本只讲三种标签与路径，文件上传作为管理员演示和选做任务。"
    ],
    "tags": [
      "HTML5",
      "媒体"
    ...
    "media": [
      {
        "type": "image",
        "url": "/media/architecture-diagram.png",
