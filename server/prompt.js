...
// 把连续空白压成一个空格、去掉首尾空格。
// ============ P4 第2课 任务 TODO 02（简单）：所有输入先洗一遍 ============
// 终端里看得到的结果：用 curl 传 receiver 为 "  林   老师  <b> "，
// 现在原样存下来；做完之后变成规整的 "林 老师"。
//
// TODO：补上三步清洗，顺序不能乱：
//   .replace(/[<>]/g, '')    去掉尖括号，别让它有机会被当成 HTML 标签
//   .replace(/\s+/g, ' ')    连续空白（空格、换行、Tab）压成一个空格
//   .trim()                  去掉首尾空格
//
// 注意 String(value ?? '') 这一层：传进来可能是 undefined、数字、甚至对象，
// 不先转成字符串，下面的 .replace 会直接报 "not a function"。
// **别信任何外部传进来的东西的类型。**
//
// 这叫**归一化**：把各种写法统一成一种。和 TODO 01 的校验是两件事——
// 归一化负责"洗干净"，校验负责"合不合规"，先洗后验。
// ==============================================================
...
export function normalizeCardInput(input = {}) {
  ...
  if (receiver.length > LIMITS.receiver)
    ...
    // ============ P4 第2课 任务 TODO 01（极简单）：把输入挡在服务端 ============
  // 终端里看得到的结果：用 curl 传一个页面上根本没有的场景，
  // 现在会被原样拼进提示词发给大模型；做完之后返回 400 INVALID_OCCASION。
  //
  // TODO：补上两条白名单校验：
  //   if (!OCCASIONS.includes(occasion)) throw Object.assign(
  //       new Error('请选择有效场景'), { status: 400, code: 'INVALID_OCCASION' })
  //   if (!TONES.includes(tone)) throw Object.assign(
  //       new Error('请选择有效语气'), { status: 400, code: 'INVALID_TONE' })
  //
  // **前端的下拉框只是方便用户，不是安全措施**——任何人都能用 curl
  // 绕过页面直接打接口。凡是"必须成立"的规则，都要在服务端再写一遍。
  //
  // 为什么用白名单而不是查坏词：坏词永远列不完，白名单只有十一项，
  // **列不全的风险为零**。
  // ==============================================================
