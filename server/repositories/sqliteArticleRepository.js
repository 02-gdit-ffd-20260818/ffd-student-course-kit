...
export function createSqliteArticleRepository(database) {
  ...
  return {
    list({ status, query = '', page = 1, pageSize = 20 } = {}) {
      ...
      const total = Number(
        database.prepare(`SELECT COUNT(*) AS count FROM articles ${clause}`).get(...params).count,
      )
      const offset = (page - 1) * pageSize
      const items = database
        .prepare(
          `SELECT ${selectColumns} FROM articles ${clause} ORDER BY published_at DESC, id DESC LIMIT ? OFFSET ?`,
        )
    ...
    find(id) {
      return toArticle(
        database.prepare(`SELECT ${selectColumns} FROM articles WHERE id = ?`).get(Number(id)),
      )
    },
    create(input) {
    ...
    update(id, input) {
      const result = database
        .prepare(
          `UPDATE articles SET slug = ?, title = ?, summary = ?, content_json = ?, tags_json = ?, status = ?, author = ?, published_at = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
        )
        .run(
          input.slug,
          input.title,
