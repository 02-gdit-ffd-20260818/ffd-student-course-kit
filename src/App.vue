...
function requestTrack(track, playNow = false) {
  ...
  const added = queue.add(item)
  const index = queue.items.findIndex(x => x.id === item.id)

  if (playNow && index >= 0) {
    playAt(index)
    notice.value = `正在播放「${titleOf(track)}」`
    return
  }
  notice.value = added
    ? `「${titleOf(track)}」已排到第 ${index + 1} 位`
    : `「${titleOf(track)}」已经在队列里了（第 ${index + 1} 位）`
