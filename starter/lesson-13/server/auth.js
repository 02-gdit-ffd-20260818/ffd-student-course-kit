...
export function createAccessToken(user, secret, now = Date.now()) {
  ...
  const payload = Buffer.from(JSON.stringify({ sub: user.id, username: user.username, role: user.role, exp: Math.floor(now / 1000) + TOKEN_TTL_SECONDS })).toString('base64url')
