...
export function createApp(options = {}) {
  ...
  app.post('/api/greetings/stream', async (req, res) => {
    let input
    try { input = normalizeCardInput(req.body) } catch (error) {
      return res.status(error.status || 400).json({ error: { code: error.code || 'INVALID_INPUT', message: error.message } })
    }
    ...
    try {
      const result = await generate(input)
      send('meta', { mode: result.mode, reason: result.reason, promptVersion: PROMPT_VERSION, requiresHumanReview: true })
      const chunks = result.text.match(/.{1,8}/gu) ?? []
