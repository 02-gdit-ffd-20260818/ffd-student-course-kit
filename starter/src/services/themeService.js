...
export function readTheme(storage) {
  try { return normalizeTheme(storage?.getItem('p1-theme')) } catch { return 'light' }
}

export function writeTheme(storage, theme) {
  const normalized = normalizeTheme(theme)
  try { storage?.setItem('p1-theme', normalized) } catch { return { theme: normalized, saved: false } }
