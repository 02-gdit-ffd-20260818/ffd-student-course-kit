...
test('generate returns a review flag and prompt version', async () => {
  await withServer(createApp({ generate: async () => ({ text: '一份祝福', mode: 'fallback', reason: 'test' }) }), async (base) => {
    ...
    assert.equal(data.promptVersion, 'greeting-card-v1.3')
  })
})

test('stream emits metadata, deltas and completion', async () => {
  await withServer(createApp({ generate: async () => ({ text: '愿日日有光', mode: 'fallback', reason: 'test' }) }), async (base) => {
    const response = await fetch(`${base}/api/greetings/stream`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ receiver: '小满', occasion: '生日', tone: '温暖', details: '' }) })
    const body = await response.text()
    assert.equal(response.status, 200)
