...
test('真实数据库：注册、登录、权限、评论及重启持久化', async () => {
 const dir=await mkdtemp(join(tmpdir(),'p2-course-'))
 const env={DATABASE_PATH:join(dir,'course.sqlite'),ADMIN_USERNAME:'teacher',ADMIN_PASSWORD:'Teacher-test-password-123'}
 const secret='test-only-secret-with-more-than-32-characters'
 let db=await openCourseDb(env), server=createCourseApp(db,secret).listen(0,'127.0.0.1')
 ...
 try {
  ...
  const [saved]=await db.query('SELECT password_hash FROM course_users WHERE username=$1',['student_a'])
  assert.notEqual(saved.password_hash,'Student-password-123')
  await new Promise(resolve=>server.close(resolve));await db.close()
  db=await openCourseDb(env);server=createCourseApp(db,secret).listen(0,'127.0.0.1');await new Promise(resolve=>server.once('listening',resolve));base='http://127.0.0.1:'+server.address().port
  ...
  const admin=await request('/api/auth/login','POST',{username:'teacher',password:env.ADMIN_PASSWORD})
  assert.equal(admin.body.data.user.role,'admin')
  const second=await request('/api/articles/1/comments','POST',{body:'教师清理验证'},other.body.data.token)
