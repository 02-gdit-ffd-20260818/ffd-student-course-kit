...
test('媒体入库、文件上传、字节一致性、范围请求与重启保存',async()=>{
 const dir=await mkdtemp(join(tmpdir(),'p2-media-')),env={DATABASE_PATH:join(dir,'blog.sqlite'),ADMIN_USERNAME:'admin',ADMIN_PASSWORD:'Admin-media-password-123'},secret='media-test-secret-with-more-than-32-characters'
 let db=await openBlogDb(env),server=createBlogApp(db,secret,{mediaPath:join(dir,'media')}).listen(0,'127.0.0.1')
 ...
 try{
  const admin=(await json('/api/auth/login','POST',{username:'admin',password:env.ADMIN_PASSWORD})).body.data.token
  ...
  const sample=await readFile(new URL('../public/media/architecture-diagram.png',import.meta.url))
  ...
  const media=[{type:'image',url,alt:'架构图',caption:'图片入库',afterParagraph:1},{type:'audio',url:'/media/learning-notes.wav',alt:'',caption:'音乐',afterParagraph:null},{type:'video',url:'/media/comment-flow.mp4',alt:'',caption:'视频',afterParagraph:2}]
  ...
  db=await openBlogDb(env);server=createBlogApp(db,secret,{mediaPath:join(dir,'media')}).listen(0,'127.0.0.1');await new Promise(resolve=>server.once('listening',resolve));base='http://127.0.0.1:'+server.address().port
