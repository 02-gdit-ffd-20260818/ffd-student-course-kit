...
if (Test-Path -LiteralPath 'kit-03-v12/starter/package.json') {
...
网络无法访问 GitHub 时，由老师发送 P1_第03课_学生资源_v1.2.zip。在文件资源管理器中右键选择“全部解压缩”，把 kit-03-v12 放进 web-work，确认里面直接有 starter/package.json，再继续第 3 步。首次安装依赖和远程推送仍需要网络。
...
} elseif (-not (Test-Path -LiteralPath '../p1-before-03.zip')) {
  Write-Output 'STOP: backup is missing.'
} elseif (-not (Test-Path -LiteralPath '../kit-03-v12/starter/package.json')) {
  Write-Output 'STOP: starter is missing.'
} else {
  # 枚举模板中的所有项，包括隐藏文件；只复制 starter 内的内容。
  Get-ChildItem -LiteralPath '../kit-03-v12/starter' -Force | ForEach-Object {
